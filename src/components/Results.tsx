import { ArrowUpRight, Users2, DollarSign, TrendingUp, Facebook } from "lucide-react";
...
const Results = () => {
  ...
  return (
    <section ref={ref} id="results" className={`section-padding bg-black relative overflow-hidden transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      ...
      {/* Subtle social icon */}
      <div className="absolute top-[20%] left-[6%] opacity-[0.03]">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-3 gap-4 sm:gap-8 mb-10 sm:mb-14 text-center">
          {stats.map((stat, index) => (
            <div key={index} className="animate-fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
              <div className="text-2xl sm:text-4xl md:text-5xl font-bold text-white mb-1 glow-pink">{stat.value}</div>
              <div className="text-xs sm:text-sm font-semibold text-white/65 mb-0.5">{stat.label}</div>
              <div className="text-[10px] sm:text-xs text-white/35">{stat.description}</div>
            </div>
          ))}
        </div>

        {/* Benefits - in a subtle card */}
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 sm:p-7 animate-fade-in hover:border-white/[0.1] transition-all duration-500">
          ...
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-3">
            {[
              ...
              <div key={index} className="flex items-center gap-2.5 text-xs sm:text-sm py-2">
                <div className="flex-shrink-0 w-5 h-5 bg-primary/15 rounded-full flex items-center justify-center">
