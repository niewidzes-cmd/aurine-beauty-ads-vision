...
const Contact = () => {
  ...
  return (
    <section ref={ref} id="contact" className={`section-padding bg-[#050505] relative overflow-hidden transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-primary/[0.06] blur-[120px] rounded-full" />
      
      {/* Decorative icons */}
      <div className="absolute top-[15%] right-[8%] opacity-[0.04]">
        <Facebook className="w-12 h-12 text-primary" />
      </div>
      <div className="absolute bottom-[15%] left-[8%] opacity-[0.03]">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        {/* Contact card */}
        <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-5 sm:p-8 animate-scale-in hover:border-white/[0.12] transition-all duration-500">
          <div className="grid sm:grid-cols-3 gap-5 sm:gap-6 mb-6 sm:mb-8">
            {[
              ...
              <a key={index} href={contact.href} className="group text-center hover:scale-105 transition-transform">
                ...
                <div className="text-[10px] sm:text-xs text-white/40 mb-0.5">{contact.title}</div>
